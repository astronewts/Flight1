/* MPU9250 Basic Example Code
 ...
 10k resistors are on the GY-9250 breakout board.
...
// See also MPU-9250 Register Map and Descriptions, Revision 4.0, RM-MPU-9250A-00, 9/12/2012 for registers not listed in
// above document; the MPU6050 and MPU 9250 are virtually identical but the latter has an on-board magnetic sensor
//
...
#define WHO_AM_I_MPU9250 0x75 //should return 0x68
...
bool initGyro()
{
  ...
  while (err && retryCount < MAX_RETRY_COUNT)
  {
    ...
    delay(100);
    err = writeByte(MPU9250_ADDRESS, PWR_MGMT_1, 0);
  }
  ...
  if (err)
  {
    Serial.print("error=");
    Serial.print(err);
    Serial.println(", we are exiting setup gyro because of failure in writeByte(MPU9250_ADDRESS, PWR_MGMT_1, 0)");
    return setupSuccess;
  }
  ...
  if (c == 0x68) // WHO_AM_I should always be 0x68
  {
    ...
    initMPU9250(); // Inititalize and configure accelerometer and gyroscope
...
void LowPowerAccelOnlyMPU6050()
{
  ...
  writeByte(MPU9250_ADDRESS, PWR_MGMT_1, c & ~0x30); // Clear sleep and cycle bits [5:6]
  writeByte(MPU9250_ADDRESS, PWR_MGMT_1, c |  0x30); // Set sleep and cycle bits [5:6] to zero to make sure accelerometer is running

  c = readByte(MPU9250_ADDRESS, PWR_MGMT_2);
  writeByte(MPU9250_ADDRESS, PWR_MGMT_2, c & ~0x38); // Clear standby XA, YA, and ZA bits [3:5]
  writeByte(MPU9250_ADDRESS, PWR_MGMT_2, c |  0x00); // Set XA, YA, and ZA bits [3:5] to zero to make sure accelerometer is running

  c = readByte(MPU9250_ADDRESS, ACCEL_CONFIG);
  writeByte(MPU9250_ADDRESS, ACCEL_CONFIG, c & ~0x07); // Clear high-pass filter bits [2:0]
  // Set high-pass filter to 0) reset (disable), 1) 5 Hz, 2) 2.5 Hz, 3) 1.25 Hz, 4) 0.63 Hz, or 7) Hold
  writeByte(MPU9250_ADDRESS, ACCEL_CONFIG,  c | 0x00);  // Set ACCEL_HPF to 0; reset mode disbaling high-pass filter

  c = readByte(MPU9250_ADDRESS, CONFIG);
  writeByte(MPU9250_ADDRESS, CONFIG, c & ~0x07); // Clear low-pass filter bits [2:0]
  writeByte(MPU9250_ADDRESS, CONFIG, c |  0x00);  // Set DLPD_CFG to 0; 260 Hz bandwidth, 1 kHz rate

  c = readByte(MPU9250_ADDRESS, INT_ENABLE);
  writeByte(MPU9250_ADDRESS, INT_ENABLE, c & ~0xFF);  // Clear all interrupts
  writeByte(MPU9250_ADDRESS, INT_ENABLE, 0x40);  // Enable motion threshold (bits 5) interrupt only

  // Motion detection interrupt requires the absolute value of any axis to lie above the detection threshold
  // for at least the counter duration
  writeByte(MPU9250_ADDRESS, MOT_THR, 0x80); // Set motion detection to 0.256 g; LSB = 2 mg
  writeByte(MPU9250_ADDRESS, MOT_DUR, 0x01); // Set motion detect duration to 1  ms; LSB is 1 ms @ 1 kHz rate

  delay (100);  // Add delay for accumulation of samples

  c = readByte(MPU9250_ADDRESS, ACCEL_CONFIG);
  writeByte(MPU9250_ADDRESS, ACCEL_CONFIG, c & ~0x07); // Clear high-pass filter bits [2:0]
  writeByte(MPU9250_ADDRESS, ACCEL_CONFIG, c |  0x07);  // Set ACCEL_HPF to 7; hold the initial accleration value as a referance

  c = readByte(MPU9250_ADDRESS, PWR_MGMT_2);
  writeByte(MPU9250_ADDRESS, PWR_MGMT_2, c & ~0xC7); // Clear standby XA, YA, and ZA bits [3:5] and LP_WAKE_CTRL bits [6:7]
  writeByte(MPU9250_ADDRESS, PWR_MGMT_2, c |  0x47); // Set wakeup frequency to 5 Hz, and disable XG, YG, and ZG gyros (bits [0:2])

  c = readByte(MPU9250_ADDRESS, PWR_MGMT_1);
  writeByte(MPU9250_ADDRESS, PWR_MGMT_1, c & ~0x20); // Clear sleep and cycle bit 5
  writeByte(MPU9250_ADDRESS, PWR_MGMT_1, c |  0x20); // Set cycle bit 5 to begin low power accelerometer motion interrupts
...
void calibrateMPU9250(float * dest1, float * dest2)
{
  ...
  writeByte(MPU9250_ADDRESS, FIFO_EN, 0x78);     // Enable gyro and accelerometer sensors for FIFO  (max size 1024 bytes in MPU-6050)
  ...
  writeByte(MPU9250_ADDRESS, XG_OFFS_USRH, data[0]);
  writeByte(MPU9250_ADDRESS, XG_OFFS_USRL, data[1]);
  writeByte(MPU9250_ADDRESS, YG_OFFS_USRH, data[2]);
  writeByte(MPU9250_ADDRESS, YG_OFFS_USRL, data[3]);
  writeByte(MPU9250_ADDRESS, ZG_OFFS_USRH, data[4]);
  writeByte(MPU9250_ADDRESS, ZG_OFFS_USRL, data[5]);
  ...
  writeByte(MPU9250_ADDRESS, XA_OFFSET_L_TC, data[1]);
  writeByte(MPU9250_ADDRESS, YA_OFFSET_H, data[2]);
  writeByte(MPU9250_ADDRESS, YA_OFFSET_L_TC, data[3]);
  writeByte(MPU9250_ADDRESS, ZA_OFFSET_H, data[4]);
  writeByte(MPU9250_ADDRESS, ZA_OFFSET_L_TC, data[5]);
...
void MPU6050SelfTest(float * destination) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
  ...
  rawData[0] = readByte(MPU9250_ADDRESS, SELF_TEST_X); // X-axis self-test results
  rawData[1] = readByte(MPU9250_ADDRESS, SELF_TEST_Y); // Y-axis self-test results
  rawData[2] = readByte(MPU9250_ADDRESS, SELF_TEST_Z); // Z-axis self-test results
