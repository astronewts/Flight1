...
int IridiumSBD::doSBDRB(uint8_t *rxBuffer, size_t *prxBufferSize)
{
   ...
   for (int i=0; i<size; ++i)
   {
      ...
      //if (stream.available())
      //{
            console(F("[FIRST check if stream available]"));
            console(F("\r\n"));
      //}
      console(F("[SECOND checking if stream available]"));
      console(F("\r\n"));
      
      if (stream.available())
      {
         console(F("[SECOND = NOPE!!!!]"));
         console(F("\r\n"));
