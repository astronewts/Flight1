...
%
[FileName,PathName,FilterIndex]=uigetfile ({'*.xls','xls-files';'*.txt','txt-files';'*.csv','csv-files'},'SELECT A RB FILE','D:\Astronewts\RB_data\');
display(char(strcat('You selected the file=>',FileName)));
FileName_wo_ext=strrep(FileName,'.txt','');
FileName_wo_ext=strrep(FileName_wo_ext,'.TXT','');
FileName_wo_ext=strrep(FileName_wo_ext,'.xls','');
FileName_wo_ext=strrep(FileName_wo_ext,'.XLS','');
FileName_wo_ext=strrep(FileName_wo_ext,'.xlsx','');
FileName_wo_ext=strrep(FileName_wo_ext,'.XLSX','');
FileName_wo_ext=strrep(FileName_wo_ext,'.csv','');
FileName_wo_ext=strrep(FileName_wo_ext,'.CSV','');
%
if strfind(FileName,'xls')
    ...
    for lw=5:size(Txt,1)
        if strcmp(char(Txt(lw,3)),'MO')==1
            ...
            Date_message_RB_PST(c,1)=cellstr(datestr(datenum(char(Date_mssg_rockblock(c,1)))-8/24));
        end
    end
end
%
...
for lw = 1:size(mssg_rockblock,1)
    ...
    for n = 1:nb_var
        if ((Format_type==1 & var_format1(n)==1)  || (Format_type==2 & var_format2(n)==1))
            ...
            %
            assignin('base',strcat(Names_Variable_MATLAB{n},'_RB'),Big_matrix(:,n));
        end
    end
